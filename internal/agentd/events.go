...
// eventsWindow is how far either side of the given moment `events` looks.
const eventsWindow = 2 * time.Hour

// eventsCmd answers "what's on around this time": every event within two hours
...
func (a *Agent) eventsCmd(args []string) (string, error) {
	...
	anchor, hasTime, err := parseAnchor(when, time.Now())
	if err != nil {
		return "", err
	}
	all, err := a.Store.ListReminders()
	if err != nil {
		return "", err
	}
...
// eventsReport is the pure formatting behind eventsCmd (separated for testing):
