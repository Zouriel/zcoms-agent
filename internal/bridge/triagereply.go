...
func buildTriageSeed(recipients []Recipient) string {
	...
	b.WriteString("The owner also runs zc-commerce, a hosted Telegram-Stars commerce platform (merchant\n")
	b.WriteString("bots, payments, orders, refunds, subscriptions, per-store billing). You can't run shell\n")
	b.WriteString("here, but if a message concerns a store / order / refund / payout, flag it and suggest the\n")
	b.WriteString("owner check it with the `zc commerce` CLI.\n\n")
